...
class TalebookAdapter : ServerAdapter {
    override val type = ServerType.TALEEBOOK
    override val capabilities = ServerCapabilities.TALEEBOOK
    ...
    override suspend fun loginWithPassword(
        ...
        val r = api.loginWithPassword(
            ...
            geetest?.lotNumber.orEmpty(), geetest?.captchaOutput.orEmpty(),
            geetest?.passToken.orEmpty(), geetest?.genTime.orEmpty()
        )
        @Suppress("DEPRECATION")
        val httpCode = r.code()
        return mapAuthResponse(httpCode, r.body(), r.errorBody()?.string(), "password", username)
    }

    override suspend fun loginGuest(captchaCode: String, geetest: GeetestParams?): ServerLoginResult {
        val r = api.loginWithPassword("", "", captchaCode,
            geetest?.lotNumber.orEmpty(), geetest?.captchaOutput.orEmpty(),
            geetest?.passToken.orEmpty(), geetest?.genTime.orEmpty())
        @Suppress("DEPRECATION")
        val httpCode = r.code()
        return mapAuthResponse(httpCode, r.body(), r.errorBody()?.string(), "guest", "访客")
    }
    ...
    override suspend fun unlockSite(
        ...
        val r = api.loginWithCode(inviteCode, captchaCode,
            geetest?.lotNumber.orEmpty(), geetest?.captchaOutput.orEmpty(),
            geetest?.passToken.orEmpty(), geetest?.genTime.orEmpty())
        @Suppress("DEPRECATION")
        val httpCode = r.code()
        val body = r.body()
        return when {
            !r.isSuccessful -> ServerLoginResult.Failure("站点访问码校验失败 (HTTP $httpCode)")
            body == null -> ServerLoginResult.Failure("站点访问码校验失败")
            ...
            body.err == "captcha.invalid" -> ServerLoginResult.Failure("人机验证失败")
    ...
    private fun mapAuthResponse(
        ...
        if (httpCode != 200 || body == null) {
            return ServerLoginResult.Failure(body?.msg ?: errorText ?: "登录失败 (HTTP $httpCode)")
        }
        if (body.err != "ok") return ServerLoginResult.Failure(body.msg ?: body.err)
    ...
    override suspend fun getIndex(): Result<IndexData> = runCatching {
        ...
        ) else throw IllegalStateException(b?.msg ?: "Unknown error")
    }
    ...
    override suspend fun toggleShelf(bookId: Int, inShelf: Boolean): Result<Unit> = runCatching {
        val resp = api.toggleShelf(bookId, ShelfToggleRequest(shelf = inShelf))
        val b = resp.body()
